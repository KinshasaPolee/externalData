...
export async function favourite(imageId) {
    try {
        const checkFavoriteResponse = await axios.get(`https://api.thecatapi.com/v1/favourites`, {
            params: {
                image_id: imageId,
            },
        });

        const isFavorited = checkFavoriteResponse.data.length > 0;

        if (isFavorited) {

            await axios.delete(`https://api.thecatapi.com/v1/favourites/${checkFavoriteResponse.data[0].id}`);
            console.log(`Removed favorite for image ID: ${imageId}`);
        } else {

            await axios.post('https://api.thecatapi.com/v1/favourites', {
                image_id: imageId,
            });
            console.log(`Added favorite for image ID: ${imageId}`);
        }
    } catch (error) {
